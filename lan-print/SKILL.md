---
name: lan-print
description: 局域网云驱直连打印（临时应急方案）— 发现网络打印机、匹配驱动、云端渲染、9100 端口下发。仅在企业尚未开通云打印（未初始化且无法共享打印机）时临时使用。
env_vars:
...
> ⚠️ **本技能是临时应急方案**，不是默认路径。
>
> **什么时候才用**：你的企业还没开通云打印（打印服务器未初始化、暂时无法让管理员共享打印机），但你急着打印文件。
> **什么时候不用**：企业已开通云打印 → 请走 [[cloud-print]]；企业没开通但有管理员可联系 → 请管理员按 [[init-server]] 开通，开通后即可云打印。
>
> 用本方案打印前，请明确告知用户这是临时方案，企业开通云打印后应回到云打印流程。

本流程帮你把文件打到局域网里的一台网络打印机（支持 9100 端口）。你需要做的：发现打印机 → 匹配驱动 → 上传文件 → 云端渲染 → 把打印数据发给打印机。
...
- 所有请求携带 Headers：
  - `tid: cdf_ai_terminal`
  - `ttp: AI`
  - 如果配置了 `WEBPRINTER_ACCESS_TOKEN`，附加 `Authorization: Bearer ***`
